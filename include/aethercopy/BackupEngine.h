...
class BackupEngine : public std::enable_shared_from_this<BackupEngine> {
  public:
    // BackupEngine(ThreadPool &pool, ICopier &copier, FormatFilter &filter, IArchiveHandler
    // &handler,
    //              const std::string &targetBase, const std::string &tempDir);

    BackupEngine(std::shared_ptr<ThreadPool> pool, std::shared_ptr<ICopier> copier,
