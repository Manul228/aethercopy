...
int copy_file(struct io_uring* ring, off_t insize)
{
    ...
    while (insize || write_left)
    {
        ...
        while (write_left)
        {
            ...
            else if (cqe->res != static_cast<int>(data->iov.iov_len))
            {
                /* short read/write; adjust and requeue */
                data->iov.iov_base =
                    static_cast<char*>(data->iov.iov_base) + cqe->res;
             ...
             * В оригинальном коде тут баг. Код правильный в случае чтения,
             * но не при записи.
             * Ну и data не освобождалась
             * */

            if (data->read)
            {
                queue_write(ring, data);
                // не трогаем write_left здесь
                reads--;
                writes++;
            }
            else
            {
                write_left -= data->first_len; // теперь здесь, когда write
                                               // реально завершён
                free(data);
                writes--;
            }
